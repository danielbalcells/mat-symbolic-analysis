function outputnmat = markovGeneratorv4(extendedNmat, nNotes)
...
% chains.
%
% Differs from markovGeneratorv3 in that it filters candidates by note
% duration.
%
...
%   (i.e. chord) and current long-term root (i.e. key).
    ...
    % Used to only use candidate notes with duration within a certain range
    % of the current note's duration.
    durMargin = 0.2;
    ...
    for i=2:nNotes
        ...
        noteCandidates = markovNotes(   markovNotes(:,1)==prevNote &...
                                        markovNotes(:,3)/currentDur<1+durMargin &...
                                        markovNotes(:,3)/currentDur>1-durMargin &...
                                        markovNotes(:,4)==currentNote &...
                                        markovNotes(:,6)/currentDur<1+durMargin &...
                                        markovNotes(:,6)/currentDur>1-durMargin,...
                                        [7,8,9]);
        % If there are no candidates using order 2, use order 1
        if isempty(noteCandidates)
            noteCandidates = markovNotes(markovNotes(:,4)==currentNote &...
                                        markovNotes(:,6)/currentDur<1+durMargin &...
                                        markovNotes(:,6)/currentDur>1-durMargin,...
                                        [7,8,9]);
        end
